# -*- coding: utf-8 -*-
"""
Created on Tue Apr 10 10:43:13 2018
...
class HSCAbstract(object):
    ...
    def __init__(self,
        ...
        self.geoms={}
        self.capacityFS=dfTable["General"].loc["targetStationSize","General"]
        self.energyDemand=pd.DataFrame(index=["Loss","electricityRES","electricityGrid","NaturalGas","Diesel"])
        self.transportType=[]
        ...
        if any(self.dfHSC["kind"]=="Storage"):
            ...
            self.dfHSC.loc[self.dfHSC["intIndex"]==self.storageIndex[0]+1,"storePart"]=True
        self.detourTruck=self.dfTable["General"].loc["detourFactorTruck","General"]
        self.detourPipeline=self.dfTable["General"].loc["detourFactorPipeline","General"]
        self.demArrFS=self.dfTable["General"].loc["utilization Station", "General"]*self.capacityFS*self.demArr**0
        self.distArrFS=self.dfTable["General"].loc["distributionDistance", "General"]*self.distArr**0
        self.nFS=self.demArr/self.demArrFS
        self.ogdenPipe=(2.43*(self.nFS**0.49))*cityRadius/self.nFS
        self.ogdenTruck=1.42*self.nFS*cityRadius/self.nFS
        self.truckDistance=[self.detourTruck*(self.distArr+self.ogdenTruck),
                            self.detourTruck*self.ogdenTruck]                
        self.pipeDistance=[self.detourPipeline*self.distArr,
                            self.detourPipeline*self.ogdenPipe]
        
        self.transportDemands=[self.demArr, self.demArrFS]
        self.TOTEX={}
        self.cumCost={}
...
#%%                      
    def calcHSC(self,
        ...
        iTrans=0
        for (key, (kind, technology, storagePartition, index)) in self.dfHSC.loc[:,[ "kind","technology","storePart", "intIndex"]].iterrows():
            capacity=self.demArr
            if kind=="Production":
                ...
                if technology=="Pipeline":
                    self.hscClasses[key]=hscClasses.PipelineSingle(self.transportDemands[iTrans],
                                                                   self.pipeDistance[iTrans],
                                                                   self.dfTable,
                                                                   costH2In=cumCost)
                    self.transportType.append("Pipeline")
                else:
                    #self.geoms[key]=self.truckDistance[iTrans].geometry
                    self.hscClasses[key]=hscClasses.Truck(self.transportDemands[iTrans],
                                                           self.truckDistance[iTrans],
                                                           technology,
                                                           self.dfTable,
                                                           costH2In=cumCost)
                ...
                if "learningRate" in self.dfTable["General"].index:
                    self.calcFSCost(self.demArr/self.capacityFS, learning = self.learning)
                self.hscClasses[key]=hscClasses.Station(self.demArrFS,
                                                         technology,
                                                         self.dfTable,
                                                         costH2In=cumCost)
            ...
            cumCost=self.hscClasses[key].getTOTEX()+cumCost
            self.TOTEX[key]=self.hscClasses[key].TOTEX
            self.cumCost[key]=cumCost
#            self.dfHSC.loc[key, "invest"]=self.hscClasses[key].invest
#
#            self.dfHSC.loc[key, "CAPEX"]=self.hscClasses[key].CAPEX
#            self.dfHSC.loc[key, "fixOPEX"]=self.hscClasses[key].fixOPEX
#            self.dfHSC.loc[key, "varOPEX"]=self.hscClasses[key].varOPEX
...
#            
            ...
            self.energyDemand[key]=[np.array([[0]])+x for x in self.hscClasses[key].getDemandAbstract()]
#        #Add the energy demand to the results 
        self.dfHSC["TOTEX"]=pd.Series(self.TOTEX)
...
#%%
    def createResFolders(self, pathResults, savePlot=False, saveResults=False):
        '''
        creating the result folders for storing the results
        '''
...
#%%    
    def plotHSC(self,
                ...
                show=True):
        '''
        plot the scenario results onto a geospatial map
        '''
